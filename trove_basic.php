...
<html>
	<head>
		<title>DECO1800 Dazed and Confused Starting Point - php search</title>
	...
	<div class="site-main" id="main">
			...
			<div class="entry-content wide">			<h2>Dazed and Confused Search</h2>
				    <p>I had a read of  <a href="http://help.nla.gov.au/trove/building-with-trove/api-technical-guide">http://help.nla.gov.au/trove/building-with-trove/api-technical-guide</a>
				    </p>
				    <p>As a result of that I decided that to search for images (following 'fake family tree' idea, but same principle applies to the history milestone idea) I should use the
                    following zones: 'picture,article,collection,book'  and I have hard-coded to append the word 'portrait' to whatever search term youy enter, in order to be more likely to find family photos.</p>
					...
					<form action="#" id="searchTrove" role="form">
						<p>
				        <label class="control-label col-lg-3" for="searchTerm">Family surname</label>&nbsp;<input id="searchTerm" type="text" />&nbsp;
		...
		<footer class="site-footer"><div class=" site-info">DECO1800 Design Computing Studio One</div></footer>
	...
	//	    var searchTerm = $("#searchTerm%20portrait").val();
	    ...
	    // Set the search zone - alternatively you can set this using a form input
		    ...
		    */	
		    ...
		    //    if (data.response.zone[0].records.work){
		        	// For each result returned, call getPicture to retrieve the transcribed text
		   //         $.each(data.response.zone[0].records.work, getPicture);
...
function getPicture(index, item){
	/*
	* Construct the search URL
	* Using the item ID, set the inclusions for the result - using the include parameter
	*/
	var picURL = "http://api.trove.nla.gov.au/work/"
	+ item.id 
	+ "?key="+apiKey;
	// Pull what values you can from the original result - article title & URL
	var title = item.heading;
	...
	// Perform the search
	$.getJSON(encodeURI(picURL), function(data){
		// Create a div element
		var info = document.createElement("div");
		// Give it the Bootstrap class panel
		$(info).addClass("panel");
